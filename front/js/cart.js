...
function countQtyTotal(pannier) {
    ...
    for (let article of pannier) {
        qtyTotal += article.qty
    }
    return qtyTotal
}

/* Fonction pour calcul total du prix du pannier */
function countPriceTot(pannier) {
    let totPrice = 0
    for (let articlePrice of pannier) {
        totPrice += articlePrice.qty * articlePrice.price
    }
    return totPrice
}
...
async function affichagePannier() {
    ...
    for (let i = 0; i < pannier.length; i++) {
        ...
        tableauPannierArticle.querySelector('.itemQuantity').addEventListener('click', function () {
            ...
            article.qty = newQty
            Total.innerHTML = `<span>${countQtyTotal(pannier)}<\span>`
            total.innerHTML = `<span>${countPriceTot(pannier)}<\span>`
            let pannierAStocker = JSON.stringify(pannier);
            localStorage.pannier = pannierAStocker;
        })
